...
const ScoreBoard: React.FC = () => {
  ...
  const { inningData, isSecondInning, iswinner, setIsWinner, match_data, setStrike } = useScoring();
  ...
  useEffect(() => {
    ...
    const latestInning: Inning = inningData[inningData.length - 1] as Inning;
          ...
          <div className="space-y-4 p-4 rounded-xl max-w-full overflow-x-auto">
            ...
            {activeBatsmen.length === 0 ? (
              ...
              activeBatsmen.map((batsman, idx) => (
                <div
                  key={idx}
                  className={`border rounded-xl p-4 transition-shadow duration-200 hover:shadow-md whitespace-nowrap ${batsman.isStriker
